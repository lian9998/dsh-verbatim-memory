...
The child is started through `recallProvider` (default `fork`) with `maxDepth: 1` and `toolFilter: { allow: [] }`. That empty list is exact, not a placeholder: `restrict()` filters only the tools a scope *inherits* (the deployment's globals and the parent's preset plane) and never the scope's own layer, so the child keeps exactly its own-scope tools — this plugin's read-only `memory_list`, `memory_search`, `memory_read`, and `memory_ask`, the delegation runtime's `structured_output`, and the harness's per-agent `subagent` tool. No file or shell tools, and `memory_recall` is not registered for a subagent child at all, so a memory recall cannot recurse. It is asked for one line plus `{seq, quote}` pairs.
